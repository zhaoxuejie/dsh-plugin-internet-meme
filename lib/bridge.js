	...
	(() => {
		...
		function previewButton() {
			...
			node.addEventListener("click", async () => {
				try {
					await fetch(PREVIEW_PATH, { method: "POST" });
				} catch {
					show("预览弹幕已就位", "tool-call");
				}
			});
			return node;
		}
		function renderSettingsSection(section) {
			...
			section.append(control("最大同屏条数", "达到上限时自动让最早的字幕淡出，避免遮挡。", select(String(settings.maxVisible), [
				["3", "3 条"],
				["4", "4 条"],
				["5", "5 条"]
			], (next) => {
			...
			section.append(control("诊断日志", "仅在浏览器控制台输出匿名事件元数据。", checkbox(settings.diagnostics, (next) => {
			...
			if (settings.theme === "custom") {
				...
				custom.append(text("自定义文案池（每行一条；可用 {tool} 代表工具名）", "dsh-meme-setting-label"));
		...
		function connect() {
			...
			source.onmessage = (event) => {
				try {
					...
					show(messageFor(pulse), pulse.kind);
				} catch (error) {
