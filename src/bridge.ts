...
(() => {
  ...
  const pulseVisuals: Record<PulseKind, { icon: string, tone: string }> = {
    ...
    'step-start': { icon: '🧠', tone: 'think' },
    'tool-call': { icon: '🛠️', tone: 'tool' },
    'tool-result': { icon: '✨', tone: 'result' },
    'turn-end': { icon: '✅', tone: 'done' },
  }

  const themes: Record<string, { label: string, lines: Theme }> = {
    classic: { label: '经典热梗', lines: {
  ...
  const settingsSections = new Set<HTMLElement>()
  let settings: Settings = loadSettings()
  let root: HTMLDivElement | undefined
  let lane: HTMLDivElement | undefined
  let live: HTMLDivElement | undefined
  ...
  function applyVisualSettings() {
    if (!root) return
    root.dataset.enabled = String(settings.enabled); root.dataset.density = settings.density; root.dataset.font = settings.fontSize; root.dataset.colorMode = settings.colorMode
  ...
  function mount() {
    ...
    lane = document.createElement('div'); lane.className = 'dsh-meme-lane'
    live = document.createElement('div'); live.className = 'dsh-meme-live'; live.setAttribute('aria-live', 'polite')
    root.append(lane, live); document.body.append(root); applyVisualSettings()
  }
  ...
  function launchInterval() { return settings.density === 'quiet' ? 3100 : settings.density === 'busy' ? 1700 : 2400 }
  function lifetime() { return settings.density === 'busy' ? 8500 : settings.density === 'quiet' ? 12000 : 10500 }
  function scheduleNextMeme() {
    ...
    if (settings.showIcons) {
      const icon = document.createElement('span'); icon.className = 'dsh-meme-item-icon'; icon.textContent = visual.icon; item.append(icon)
    }
  ...
  function control(label: string, hint: string, input: HTMLElement) {
    const row = document.createElement('label'); row.className = 'dsh-meme-setting-row'
    const copy = document.createElement('span'); copy.className = 'dsh-meme-setting-copy'
    copy.append(text(label, 'dsh-meme-setting-label'), text(hint, 'dsh-meme-setting-hint')); row.append(copy, input); return row
  }
  function select(value: string, options: Array<[string, string]>, onChange: (next: string) => void) {
    const node = document.createElement('select'); node.className = 'dsh-meme-select'
    for (const [optionValue, label] of options) { const option = document.createElement('option'); option.value = optionValue; option.textContent = label; option.selected = optionValue === value; node.append(option) }
    node.addEventListener('change', () => onChange(node.value)); return node
  }
  function checkbox(checked: boolean, onChange: (next: boolean) => void) {
    const node = document.createElement('input'); node.type = 'checkbox'; node.className = 'dsh-meme-checkbox'; node.checked = checked
    node.addEventListener('change', () => onChange(node.checked)); return node
  }
  ...
  async function runPreview() {
    ...
    if (settings.soundEnabled) playAudioFeedback({ kind: 'tool-call' })
  ...
  function clearDetachedSettingsSections() {
    for (const section of settingsSections) {
      if (!section.isConnected) settingsSections.delete(section)
    }
  }
  function getNativeOptions(content: HTMLElement) {
    return Array.from(content.children).find((child, index) => index > 0 && child.id !== SETTINGS_PANE_ID) as HTMLElement | undefined
  }
  function restoreNativeSettings(content: HTMLElement) {
  ...
  function showMemeSettings(content: HTMLElement) {
    const nativeOptions = getNativeOptions(content)
    if (!nativeOptions) return
    nativeOptions.style.display = 'none'
    if (content.querySelector(`#${SETTINGS_PANE_ID}`)) return
    const pane = document.createElement('div'); pane.id = SETTINGS_PANE_ID; pane.className = 'dsh-meme-settings-pane'
    const section = document.createElement('section'); section.id = SETTINGS_ID; section.className = 'dsh-meme-settings'
    settingsSections.add(section); renderSettingsSection(section); pane.append(section); content.append(pane)
  }
  function injectSettingsNavigation() {
    const dialog = document.querySelector<HTMLElement>('[role="dialog"][aria-modal="true"]')
    const nav = dialog?.querySelector('nav')
    const content = nav?.nextElementSibling as HTMLElement | null
    if (!nav || !content) { clearDetachedSettingsSections(); return }
    ...
    if (!navList) return
    let memeNav = navList.querySelector<HTMLButtonElement>(`#${SETTINGS_NAV_ID}`)
    if (!memeNav) {
      ...
      memeNav.addEventListener('click', () => { memeSettingsOpen = true; injectSettingsNavigation() })
    ...
    for (const button of nativeButtons) {
      ...
      button.addEventListener('click', () => { memeSettingsOpen = false; injectSettingsNavigation() })
    }
